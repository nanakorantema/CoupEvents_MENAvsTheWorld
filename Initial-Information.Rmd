...
```{r}
fit_event_type <-   stan_glm(formula = realized ~ event_type,
                         data = Clean_coup,
                         family = "binomial",
                         refresh = 0)

fit_2 <- stan_glm(formula = realized ~  popular + dissident + foreign + military + rebel + palace + auto,
                         data = Clean_coup,
                         family = "binomial",
                         refresh = 0)

print(fit_event_type)
print(fit_2)
```
...
```{r}
#facet the distribution for fit_2

newobs2 <- tibble( popular = rep(c(1,0)), dissident = rep(c(1,0)), foreign = rep(c(1,0)),
                   military = rep(c(1,0)), rebel = rep(c(1,0)), palace = rep(c(1,0)), 
                   auto = rep(c(1,0)),
                   names = paste(popular, dissident, foreign, military, rebel, 
                                 palace, auto, c(1,0), sep = "_"))
newobsx <- tibble( popular = 1, dissident =  1, foreign =  1,
                   military =  1, rebel = 1, palace =  1, auto =  1))

                  set_names(popular, dissident, foreign, military, rebel, 
                                 palace, auto) 
                  
                  
newobsz <- tibble( popular = c(1,0), dissident = c(1,0), foreign = c(1,0),
                   military = c(1,0), rebel = c(1,0), palace = c(1,0), auto = c(1,0)) 
...
pe_2 <- posterior_epred(fit_2, 
                 newdata = newobs2) %>% 
                 as_tibble() 
 
                 
#popular, dissident, foreign, military, rebel,palace, auto
                             

pe_x <- posterior_epred(fit_2, 
                 newdata = newobsx) %>% 
                 as_tibble() 
...
```
 military = rep(c(1,0)), rebel = rep(c(1,0)), palace = rep(c(1,0)), auto = rep(c(1,0)) , names = paste(popular, dissident, foreign, military, rebel, palace, auto)


```{r}
newobs <- (tibble(event_type = c("coup", "attempted", "conspiracy")))

pe_eventype <- posterior_epred(fit_event_type, 
                 newdata = newobs) %>% 
                 as_tibble() %>% 
                 rename(Attempted = `1`) %>% 
                 rename(Conspiracy = `2`) %>% 
                 rename(Coup = `3`) %>% 
                 pivot_longer(cols = c(Attempted, Conspiracy, Coup),
                              names_to =  "event_type",
                              values_to = "success") 

pe_eventype %>% 
  ggplot(aes(x = success, fill = event_type)) +
    geom_dotplot() +
     labs(title = "Posterior for Coup Success",
         subtitle = "which is most likely to be successful?",
         caption = "From Coup Data Set",
         x = "Event Type",
         y = "Probability") + 
