...
newobs3 <-  tibble(event_type = rep(c("coup", "conspiracy", "attmepted"), 2),
                    killed = rep(c(1,0), 3)) %>% 
                    mutate(names = paste(event_type, killed, sep = "_"))

pe_3 <- posterior_epred(fit_3, 
                         newdata = newobs3) %>% 
                         as_tibble() 
                         set_names(newobs3$names) 
                         pivot_longer(names_to = "event_type",
                                      values_to = "perc_realized",
                                      cols = c("coup_1" : "attemtped_0")) 
                         separate(event_type, into = c("") 
        ```
